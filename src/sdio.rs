...
impl Logger {
...

        {
                ...
                .ok()
            {
                if let Some(flash) = flash.as_mut() {
                    let _ =
                        queue::push(flash, LOGS_FLASH_RANGE, &mut NoCache::new(), &buffer, false)
                            .await;
                }
    ...
    pub async fn get_logs<'a>(&'a mut self, f: impl Fn(&'_ str)) {
        ...
        loop {
            ...
            let mut iterator = queue::iter(flash, LOGS_FLASH_RANGE, &mut no_cache)
                .await
                .unwrap();
